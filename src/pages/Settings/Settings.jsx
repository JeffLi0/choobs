...
function Settings(props) {
		...
		const fetchUserData = async () => {
			...
			try {
				if (
					...
					sessionStorage.getItem("userData").pfp
				) {
					setUserData(JSON.parse(sessionStorage.getItem("userData")));
					setPreviewPFP(
						JSON.parse(sessionStorage.getItem("userData")).pfp,
					);
				} else {
					...
					if (userData) {
						...
						setEmailNotifications(
							userDataSnapshot.data().preferences
								.emailNotifications,
						);
			...
			<div className={styles.colorPicker}>
				{colors.map((color, index) => (
					<button
						...
						onClick={() => updatePreviewPFP(color)}
					>
						{previewPFP[0] === color && (
	...
	return (
		<>
			<div
				className={`${styles.header} ${!isMobile ? styles.desktop : ""}`}
			>
				{isMobile ? <h3>Settings</h3> : <h2>Settings</h2>}
			</div>
			<div
				className={`${styles.settings} ${!isMobile ? styles.desktop : ""}`}
			>
				{loading ? (
					...
					<>
						...
						<div className={styles.category}>
							...
							<label
								...
								onClick={toggleEmailNotifcations}
							>
								<span onClick={(e) => e.preventDefault()}>
									Receive friend request email notifications
								</span>
								<label
