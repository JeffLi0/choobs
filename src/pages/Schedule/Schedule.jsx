      ...
      try {
        ...
        if (
          ...
          const updatedCacheOrder = [
            ...(dateKey === todayKey ? [dateKey] : [todayKey, dateKey]),
            ...cacheOrder.filter(
  ...
  useEffect(() => {
    const updateHighlightedEvents = () => {
      ...
      const updatedEvents = events.map((event) => {
        ...
        const isHighlighted =
          currentTime >= eventStartDate && currentTime <= eventEndDate;
        ...
        <div className={styles.countdown}>
          <div className={styles.countdownText}>
            <i>
              {minutesUntilStart +
                1 +
                (minutesUntilStart !== 0 ? " minutes" : " minute") +
                " until start"}
            </i>
            <i>0%</i>
          </div>
      ...
      if (timeDifference >= 60000) {
        ...
        return (
          <div className={styles.countdown}>
            <div className={styles.countdownText}>
              <i>
                {Math.floor(timeDifference / 60000) + 1 + " minutes remaining"}
              </i>
      ...
      <div className={isMobile ? styles.header : styles.desktopHeader}>
        ...
        <div className={styles.calendar}>
          ...
          <div className={styles.week}>
            ...
            <button
              className={`${styles.button} ${styles.day} ${isWeekend(displayDate.getDate()) ? styles.grayed : ""} ${styles.today}`}
            >
        ...
        <div className={styles.ul}>
          {loading ? (
            ...
            events
              .filter((event) => {
                if (event.start.dateTime && event.end.dateTime) {
                  const hasHalfDay = fullDayEvent.some((element) =>
                    element.includes("Half Day"),
                  );
                  ...
                  if (!hasHalfDay) {
                    ...
                    if (/^Lunch \d+$/.test(event.summary)) {
                      const lastBlock = events.find((event) =>
                        ["F1", "D2", "C2", "F3", "D4", "C4"].includes(
                          event.summary,
                        ),
                      ).summary;
                    ...
                    if (
                      !blocksRendered.includes(event.summary.replace("$", ""))
                    ) {
              ...
              .map((event) => {
                const isHalfDay = fullDayEvent.some((element) =>
                  element.includes("Half Day"),
                );
                    ...
                    if (lastBlock && scheduleData.length > 0) {
                      ...
                      if (!(lastBlockRoom > 899 || lastBlockRoom < 100)) {
                        if (lastBlockRoom >= 500) {
                          ...
                          events[
                            events.findIndex(
                              (event) => event.summary === "Lunch 1",
                            )
                          ].start.dateTime = new Date(
                            displayDate.setHours(10, 55, 0, 0),
                          ).toISOString();
                          events[
                            events.findIndex(
                              (event) => event.summary === "Lunch 1",
                            ...
                            displayDate.setHours(11, 25, 0, 0),
                          ).toISOString();
                          events[
                            events.findIndex(
                              (event) => event.summary === lastBlock,
                            )
                          ].start.dateTime = new Date(
                            displayDate.setHours(11, 30, 0, 0),
                          ).toISOString();
                        } else if (
                          lastBlockRoom < 500 ||
                          lastBlockRoom.toLowerCase() === "gym"
                  ...
                  <React.Fragment key={event.id}>
                    {
                      ...
                      matchingData ? (
                        ...
                        <>
                          ...
                          {currentDay &&
                            ...
                            blocksRendered.every(
                              (str) => !str.includes("Lunch"),
                            ) && (
                              <div className={`${styles.lunchError}`}>
                                <div className={styles.warning}>
                                  ...
                                  <span>
                                    Your lunch could not be calculated.
                                  </span>
                                </div>
                              </div>
                            )}
                      ...
                      scheduleData.length > 0 &&
                        ...
                        Object.values(scheduleData).some(
                          (value) =>
                            value.block ===
                              event.summary.replace(/([A-Z])(\d+)/, "$1$$$2") ||
