...
function Social() {
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      try {
        const relevantFriendUIDs = friendData
          .filter((friend) => friend[2] === 0 || friend[2] === 3)
          .map((friend) => friend[1]);
        let readCount = 0;
        ...
        if (relevantFriendUIDs.length !== 0) {
          ...
          for (let i = 0; i < relevantFriendUIDs.length; i += 30) {
            ...
            querySnapshot.forEach((doc) => {
              const scheduleData = Object.entries(doc.data().classes || {}).map(
                ([block, classNames]) => ({
                  block,
                  classNames,
                }),
        ...
        const currentDayEvents = response.data.items.filter((event) => {
          ...
          return (
            (eventStartDate.getDate() === displayDate.getDate() &&
              eventStartDate.getMonth() === displayDate.getMonth() &&
              eventStartDate.getFullYear() === displayDate.getFullYear()) ||
            isFullDayEvent
          );
        });
    ...
    const updateActiveBlocks = () => {
      ...
      const activeEvents = events.filter((event) => {
        ...
        if (event.summary.includes("Lunch")) {
          ...
          return (
            currentTime >= new Date(eventStartDate.getTime() - 5 * 60 * 1000) &&
  ...
  const sendRequest = async (friendUID) => {
    try {
      ...
      if (friendData.preferences.emailNotifications) {
        ...
        fetch(
          "https://script.google.com/macros/s/AKfycbyD-LlTav4EIE3bsIAigpf-54LR-S_X6E581Ua8Wi01RevWF2HvEeGXspuPGBlM-Ixu/exec",
          {
            method: "POST",
            body: formDatab,
          },
  ...
  const handleSearch = useCallback(
    async (e) => {
      ...
      if (searchedUser.trim() === "") {
        ...
        searchedUser.trim().length < 3 ||
        (/\d/.test(searchedUser) && searchedUser.length < 8)
      ) {
            ...
            if (searchedUser === "everyone") {
              ...
              if (
                doc
                  .data()
                  .email.toLowerCase()
                  .substring(0, doc.data().email.toLowerCase().indexOf("@"))
                  .includes(searchedUser.split("@")[0].toLowerCase())
              ) {
                results.push([
                  doc.data().name,
                  doc.data().email,
  ...
  return (
    <>
      <div
        className={`${isMobile ? styles.header : styles.desktopHeader} ${friendScheduleUID ? styles.active : ""}`}
      >
        {!friendScheduleUID ? (
          <div>
            <div>
      ...
      {showRemoveModal && (
        <div
          className={`${styles.modalContainer} ${removeModalFade && styles.fade} ${isMobile && styles.mobileModal} ${styles.unfriendModal}`}
          onClick={closeModal}
        >
          <div
            className={`${styles.modalContent} ${removeModalFade ? styles.slide : ""}`}
            onClick={(e) => e.stopPropagation()}
          >
            <h3>
              Unfriend{" "}
              {friendData.find((friend) => friend[1] === showRemoveModal)[0]}
            </h3>
            <p>
            ...
            <div>
              ...
              <button onClick={() => unfriend(showRemoveModal)}>
                Unfriend{" "}
                {friendData.find((friend) => friend[1] === showRemoveModal)[0]}
              </button>
            </div>
      ...
      <div
        className={`${isMobile ? styles.social : styles.desktopSocial} ${friendScheduleUID ? styles.ios : ""}`}
      >
        {!friendScheduleUID ? (
          <div className={styles.friendsList}>
            {loading ? (
              ...
              friendData
                .sort((a, b) => {
                  ...
                  return friendData
                    ...
                    .localeCompare(
                      friendData.find((friend) => friend[1] === b[1])[0],
                    );
                })
                .filter((friend) => {
                    ...
                    <React.Fragment key={friend[1]}>
                      ...
                      >
                        <div className={styles.friendInfo}>
                          ...
                          {friend[2] === 3 && (
                            <div
                              className={`${"material-symbols-rounded"} ${styles.pin}`}
                            >
                              &#xe6aa;
                            </div>
                          )}
                          <span>
                            ...
                            {activeBlocks.length > 0 &&
                              friendSchedules[friend[1]] && (
                                <span>
                                  {activeBlocks.map((block) => {
                                    const currentTime = new Date();
                                    let currently;
                                    if (
                                      new Date(block.start.dateTime) >
                                      currentTime
                                    ) {
                                    ...
                                    if (
                                      !events[0].summary.includes("Half Day")
                                    ) {
                                      const className = friendSchedules[
                                        friend[1]
                                      ].find(
                                        (item) => item.block === block.summary,
                                      );
                                      if (className) {
                                        return (
                                        ...
                                        (block.summary.includes("Lunch") ||
                                          block.summary.includes("I-block") ||
                                          block.summary.includes("Advisory")) &&
                                        !friendSchedules[friend[1]].find(
                                          (item) =>
                                            activeBlocks.find(
                                              (activeBlock) =>
                                                activeBlock.summary ===
              ...
              <div className={styles.noFriends}>
                ...
                <p>
                  Invite your friends to sign up at <span>choobs.app</span> and
                  send them a friend request using the button in the top right.
                </p>
              </div>
            )}
          </div>
      ...
      {showAddModal && (
        <div
          className={`${styles.modalContainer} ${addModalFade && styles.fade} ${isMobile && styles.mobileModal}`}
          onClick={closeModal}
        >
          <div
            className={`${styles.modalContent} ${addModalFade ? styles.slide : ""}`}
            onClick={(e) => e.stopPropagation()}
          >
            <div>
              <h3>Add friends</h3>
              ...
              <div className={styles.pendingFriends}>
                {searchResults.length > 0 ? (
                  ...
                  <>
                    ...
                    {(/\d/.test(searchedUser) ||
                      searchedUser.includes("stu")) &&
                      !/^\d{2}stu\d{3}/.test(searchedUser) && (
                        <i>
                          To search by email, use the entire first part (e.g.,
                          “00stu000”).
                        </i>
                      )}
                  </>
                    ...
                    return (
                      <div className={styles.pendingFriend} key={friend[1]}>
                        ...
                        <div className={styles.buttons}>
                          <button
                            onClick={() => dismissRequest(friend[1], true)}
                          >
                            Ignore
                          </button>
                          <button onClick={() => acceptRequest(friend[1])}>
                            Accept
                          </button>
                        </div>
                      </div>
                  ...
                  .map((friend) => {
                    return (
                      <div className={styles.pendingFriend} key={friend[1]}>
                        ...
                        <div className={styles.buttons}>
                          <button
                            onClick={() => dismissRequest(friend[1], false)}
                          >
                            Cancel
                          </button>
                        </div>
