				...
				if (relevantFriendUIDs.length !== 0) {
					...
					for (let i = 0; i < relevantFriendUIDs.length; i += 30) {
						...
						querySnapshot.forEach((doc) => {
							const scheduleData = Object.entries(
								doc.data().classes || {},
							).map(([block, classNames]) => ({
								block,
								classNames,
							}));
			...
			const activeEvents = events.filter((event) => {
				...
				if (event.summary.includes("Lunch")) {
					...
					return (
						currentTime >=
							new Date(
								eventStartDate.getTime() - 5 * 60 * 1000,
							) && currentTime <= eventEndDate
					);
				}
			});
				...
				>
					...
					>
						<h3>
							...
							{
								friendData.find(
									(friend) => friend[1] === showRemoveModal,
								)[0]
							}
						</h3>
						<p>
									...
									<>
										...
										{(/\d/.test(searchedUser) ||
											...
											!/^\d{2}stu\d{3}/.test(
												...
												<i>
													To search by email, use the
													entire first part (e.g.,
													“00stu000”).
												</i>
											)}
									</>
