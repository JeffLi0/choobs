	...
	const generateICS = (events) => {
		if (scheduleData) {
			...
			lunchBlocks.forEach((id) => {
				if (scheduleData.some((item) => item.block === id)) {
					...
					if (
						...
						["D1", "H1", "H2", "D3", "H3", "H4"].indexOf(id) > -1
					) {
						classesToDelete.push(
			...
			events = events.filter(
				(event) => !classesToDelete.includes(event.summary),
			);
			...
			for (const [date, dayEvents] of Object.entries(eventsByDate)) {
				const fullDayEvent = dayEvents.find(
					(event) => !event.start.dateTime,
				);
				const isHalfDay =
					fullDayEvent && fullDayEvent.summary.includes("Half");

				if (!isHalfDay) {
					if (
						dayEvents.some((event) =>
							/^(C|G)\$[1-4]$/.test(event.summary),
						)
					) {
					...
					try {
						const lastBlockID = dayEvents.find((event) =>
							["F1", "D2", "C2", "F3", "D4", "C4"].includes(
								event.summary,
							),
						).summary;
						const lastBlockRoom = scheduleData.find(
							(event) => event.block === lastBlockID,
						).classNames[1];
						const lastBlockEvent = dayEvents.find(
							(event) => event.summary === lastBlockID,
						);
						...
						if (
							...
							if (lastBlockEvent) {
								if (
									...
									lastBlockEvent.end.dateTime = new Date(
										new Date(
											lastBlockEvent.end.dateTime,
										).setHours(11, 25, 0, 0),
									).toISOString();
								} else {
									dayEvents.forEach((event) => {
									...
									lastBlockEvent.start.dateTime = new Date(
										new Date(
											lastBlockEvent.end.dateTime,
										).setHours(11, 30, 0, 0),
									).toISOString();
								...
								const index = events.findIndex(
									(event) => event.id === lastBlockEvent.id,
								);
								if (index !== -1) {
			...
			events.forEach((event) => {
				...
				const matchingData = scheduleData.find((data) => {
					return (
						event.summary.includes(data.block.replace("$", "")) ||
						event.summary.includes(data.block)
					);
				});
	...
	return (
		<>
			<div className={styles.category}>
				...
				<span>
					View your schedule on Google Calendar or your preferred
					calendar app.
				</span>
			...
			{showModal && (
				...
				>
					...
					>
						...
						<ol>
							<li>
								Download and locate the .ics file on your
								device.
							</li>
							<li>
								Go to{" "}
							...
							<li>
								Select “Add calendar” and “Create new calendar”
								in the left sidebar.
							</li>
							<li>
								Create a new calendar (Optional but highly
								recommended).
							</li>
							<li>
						...
						<div>
							...
							>
								<div
									className={`${showRoomNumbers ? styles.checked : ""}`}
								/>
								<div>
									<div>Show Room Numbers in Event Name</div>
									<span>
										e.g., "Math (800)", "Science (300)"
									</span>
								</div>
							</div>
