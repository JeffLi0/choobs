...
function Artwork(props) {
	...
	useEffect(() => {
		if (props.loading) {
			let newArt;
			const filteredArt = artwork.filter(
				(art) => !recentArt.includes(art),
			);

			if (filteredArt.length > 0) {
				newArt =
					filteredArt[Math.floor(Math.random() * filteredArt.length)];
			} else {
		...
		<>
			{(props.events.length === 0 ||
				(props.events.length <= 1 &&
					props.fullDayEvent.every(
						(event) => !/Day [0-6].*/.test(event),
					))) && (
				<>
					<img
						className={`${styles.img} ${!props.loading ? styles.visible : ""}`}
						draggable="false"
						src={
